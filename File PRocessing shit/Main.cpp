...
void displayRecords(std::string activeFile, std::vector<Student> students)
{
    ...
    for (size_t i = 0; i < students.size(); ++i)
    {
        std::cout << i + 1 << "\t" << students[i].StudentID << "\t" 
                  << std::setw(20) << std::left << students[i].Surname << "\t"
                  << std::setw(20) << std::left << students[i].FirstName << "\t" 
        ...
        switch (mainChoice)
        {
            ...
            case '2':
            {
                ...
                if (inStudRec)
                {
                    ...
                    Student student;
                    std::string gender;
                    while (inStudRec >> student.StudentID >> student.Surname >> student.FirstName >> student.BirthDate >> gender)
                    {
